								...
								<form method="POST" action="{{ route('admin.process-items.update', enc($processItem->id)) }}">
									...
									<div class="row">
										<div class="col-sm-6">
											...
											<div class="col-sm-3"><div class="form-group"><label>Short Code <span class="required">*</span></label><input type="text" name="short_code" value="{{ old('short_code', $processItem->short_code) }}" class="form-control" required>@error('short_code')<span class="text-danger small">{{ $message }}</span>@enderror</div></div>
											<div class="col-sm-3"><div class="form-group"><label>Department</label><select name="department_id" class="form-control"><option value="">Reusable / unassigned</option>@foreach($departments as $department)<option value="{{ $department->id }}" @selected((string) old('department_id', $processItem->department_id) === (string) $department->id)>{{ $department->department_name }}</option>@endforeach</select></div></div>
											<div class="col-sm-3"><div class="form-group"><label>Display Order</label><input type="number" name="display_order" value="{{ old('display_order', $processItem->display_order) }}" class="form-control" min="0"></div></div>
										<div class="col-sm-6">
										...
										<div class="col-sm-3">
													<div class="form-group"><label>Last Serial No</label><input type="number" name="process_sl_no_last" value="{{ old('process_sl_no_last', $processItem->process_sl_no_last) }}" class="form-control" min="0"></div>
										</div>
										<div class="col-sm-3">
