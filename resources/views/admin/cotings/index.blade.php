...
<body class="hold-transition sidebar-mini">
		...
		<div class="content-wrapper">
			<section class="content-header">
				...
				<div class="header-title">
					<h1>Cotings</h1><small>Cotings list</small>
				</div>
			</section>
						...
						<div class="panel panel-bd lobidrag">
							...
							<div class="panel-body">
								<div class="row" style="margin-bottom:5px">
									<form action="{{ route('admin.cotings.index') }}" method="GET">
									...
									<div class="col-sm-3"><a href="{{ route('admin.cotings.create') }}" class="btn btn-add"><i class="fa fa-plus"></i> Add Cotings</a></div>
								</div>
								<div class="table-responsive">
									<table class="table table-bordered table-striped table-hover">
										...
										<tbody>
											@forelse ($cotings as $row)
											<tr id="cotings-row-{{ $row->id }}">
												<td>{{ $row->code }}</td>
												<td>{{ $row->name }}</td>
												<td>{{ $row->status }}</td>
												<td><a href="{{ route('admin.cotings.edit', enc($row->id)) }}"><i class="fa fa-pencil"></i></a></td>
								...
								<div class="pagination">{{ $cotings->links() }}</div>
							</div>
						</div>
					</div>
				</div>
	...
	<script>
		function deleteRecord(id, rowId) {
			...
			$.ajax({
				type: 'DELETE',
				url: '{{ url(' / admin / cotings ') }}/' + encodeURIComponent(id),
				data: {
					_token: '{{ csrf_token() }}'
				},
				success: function() {
					$('#cotings-row-' + rowId).hide();
				},
				error: function() {
