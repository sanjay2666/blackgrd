                            ...
                            <table class="table table-bordered table-striped table-hover">
                                ...
                                @forelse($packagingOrders as $order)
                                    <tr>
                                        ...
                                        <td><a class="btn btn-primary btn-xs" href="{{ route('packaging.show-order-details', enc($order->id)) }}">View</a> <a class="btn btn-default btn-xs" target="_blank" href="{{ route('packaging.print-packaging-slip', enc($order->id)) }}">Print Slip</a>@if(in_array($order->packaging_status, ['accepted','packed'])) <a class="btn btn-info btn-xs" href="{{ route('packaging.show-order-details', enc($order->id)) }}#pack-quantity">{{ $order->packaging_status === 'accepted' ? 'Pack Quantity' : 'Continue' }}</a>@endif @if($order->dispatchable_quantity > 0 && !in_array($order->packaging_status, ['cancelled'])) <a class="btn btn-success btn-xs" href="{{ route('sales-challans.create') }}">Sales Challan</a>@endif</td>
                                    </tr>
                                @empty
