        ...
        <form method="post" action="{{ route('sales-challans.store') }}" id="salesChallanForm">
            ...
            <div class="row"><div class="col-sm-8"><div class="panel panel-bd lobidrag">
                ...
                <div class="panel-body table-responsive"><table class="table table-bordered table-condensed">
                    ...
                    <tbody>@forelse($allocations as $allocation)<tr data-customer="{{ $allocation->packagingOrder->customer_id }}" data-customer-name="{{ $allocation->packagingOrder->customer?->name }}">
                        ...
                        <td><input disabled class="form-control input-sm dispatch-qty" type="number" min="0.01" max="{{ $allocation->available_to_dispatch }}" step="0.01" value="{{ $allocation->available_to_dispatch }}" data-id="{{ $allocation->id }}"></td>
            ...
            <div class="panel panel-bd lobidrag"><div class="panel-heading"><h4>Challan, Delivery and Transport Details</h4></div><div class="panel-body">
                <div class="row"><div class="col-sm-3 form-group"><label>Challan Date *</label><input class="form-control" type="date" name="challan_date" value="{{ old('challan_date', now()->toDateString()) }}" required></div>
                    ...
                    <div class="col-sm-3 form-group"><label>Transporter</label><select class="form-control" name="transporter_id"><option value="">Self / not specified</option>@foreach($transporters as $transporter)<option value="{{ $transporter->id }}" @selected(old('transporter_id') == $transporter->id)>{{ $transporter->name }}{{ $transporter->phone ? ' - '.$transporter->phone : '' }}</option>@endforeach</select></div>
