...
@media print {
    ...
    .meter-table2 th {
        background-color: #34896f !important;
        color: #fff !important;
        -webkit-print-color-adjust: exact !important;
    }
...
<div class="header">
    ...
    <div class="section-title">Mill Dispatch Challan</div>
</div>
...
<table class="meter-table2">
    ...
    <?php
        $totalMeter = 0;
        $srno = 0;
        if (!empty($smData['StockMillDispatchItem'])) {
            foreach ($smData['StockMillDispatchItem'] as $index => $item) {
                $srno = $index + 1;
                $meter = $item['insp_quan_size'] ?? 0;
                $totalMeter += $meter;
    ?>
    <tr>
        <td><?= $srno ?></td>
        <td colspan="1" ><?= number_format($meter, 2) ?></td>
...
<table class="remark-table">
    ...
    <tr>
        <td class="bold left">Amount</td>
        <td class="right">0.00</td>
    </tr>
    <tr>
        <td class="bold left">With GST Amount</td>
        <td class="right">0.00</td>
    </tr>
</table>
...
<div class="footer">
    ...
    <div>Prepared By: {!! CommonController::getUserName($smData->created_by) !!}</div>
    <div>For AJY TECH INDIA PRIVATE LIMITED</div>
</div>
