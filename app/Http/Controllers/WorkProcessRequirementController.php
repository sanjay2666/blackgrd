...
class WorkProcessRequirementController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($req_type !== null && $req_type !== '') {
            $canonicalRequirementStatus = match ((string) $req_type) {
                '0' => WorkRequirementStatus::Pending,
                '1' => WorkRequirementStatus::Accepted,
                '2' => WorkRequirementStatus::Denied,
                default => null,
            };
            if ($canonicalRequirementStatus !== null) {
                $query->withRequirementStatus($canonicalRequirementStatus);
            }
        }
    ...
    public function getWorkProcessAllotmentView(Request $request)
    {
        ...
        $dataIT = ItemType::where('item_type_id', '=', $dataWPR->item_type_id)->where('status', '=', 'Active')->first();
    ...
    public function getProcessRequirementItems(Request $request)
    {
        ...
        $procesTypeId = $dataWk->process_type_id;
        $dataWPR = WorkProcessRequirement::where('work_order_id', '=', $workOrdId)->where('status', '=', 'Active')->where('is_accept', '!=', '2')->get();
        $str = '';
        if ($procesTypeId == '2') {
    ...
    public function printJobCardGatepass($id)
    {
        ...
        $dataWPR = WorkProcessRequirement::where('req_lot_no', $reqLotNo)->where('work_order_id', $mainWorkOrderId)->where('req_fabric_type', $reqFabricType)->where('process_type_id', $processTypeId)->where('status', 'Active')->where('is_accept', '1')->get();
    ...
    public function print_warehouse_item_requirement_gatepass_by_lot($id)
    {
        ...
        $dataWPR = WorkProcessRequirement::where('req_lot_no', $reqLotNo)->where('process_type_id', $processTypeId)->where('status', 'Active')->where('is_accept', '1')->get();
        if ($dataWPR->isEmpty()) {
            abort(404, 'Work Process Requirements not found.');
        }
        $workOrderIds = $dataWPR->pluck('work_order_id')->unique();
    ...
    public function AcceptWarehouseReq(Request $request, TransitionWorkRequirement $transitionRequirement)
    {
        DB::beginTransaction();
        ...
        try {
            ...
            $obj->modified_at = now();
            $obj->save();
            $transitionRequirement->synchronize($obj->refresh());
    ...
    public function DenyWarehouseReq(Request $request, TransitionWorkRequirement $transitionRequirement)
    {
        ...
        try {
            ...
            $obj->modified_at = now();
            $obj->save();
            $transitionRequirement->deny(
                $obj,
                $request->remarks ?? $request->deny_remark ?? $request->remark ?? null,
                $IndId,
            );
                    ...
                    foreach ($dataWISList as $dataWIS) {
                        ...
                        WarehouseItemStock::whereKey($wisId)
                            ...
                            ->update([
                                ...
                                'allocation_status' => $inspQuanSize <= ($inspAllotQuanSize + $remainingQuantity) ? 'allocated' : 'partially_allocated',
                                'allot_work_order_id' => $workOrderId,
                ...
                foreach ($dataWISList as $dataWIS) {
                    ...
                    WarehouseItemStock::whereKey($wisId)
                        ...
                        ->update([
                            ...
                            'allocation_status' => $newBalanceQuantity <= 0 ? 'allocated' : 'partially_allocated',
                            'allot_work_order_id' => $workOrderId,
                            'work_pro_req_id' => $wprId,
                            'stock_alloted_by' => $individualId,
                            'alloted_remark' => $allotmentRemark,
                    ...
                    WorkProcessRequirement::where('id', '=', $wprId)
                        ->update([
                            ...
                            'is_accept' => 1,
                            'requirement_status' => $newBalanceQuantity <= 0 ? 'accepted' : 'partially_allotted',
                            'allocation_status' => $newBalanceQuantity <= 0 ? 'allocated' : 'partially_allocated',
                            'alloted_remark' => $allotmentRemark,
                            'modified_by' => $individualId,
                            'modified_at' => now(),
                ...
                if (isset($reqGreyQtyArr[$key])) {
                    ...
                    if ($dataWIS) {
                        ...
                        WarehouseItemStock::where('wis_id', $wisId)->update([
                            ...
                            'allocation_status' => $dataWIS->insp_quan_size <= ($dataWIS->insp_allot_quan_size + $remainingQuantity) ? 'allocated' : 'partially_allocated',
                            'allot_work_order_id' => $workOrderId,
                            'work_pro_req_id' => $wprnId,
                            'stock_alloted_by' => $individualId,
    ...
    public function getWorkProcessPrintingRequirement(Request $request)
    {
        ...
        $dataWPR = WorkProcessRequirement::where('id', '=', $FId)->where('status', '=', 'Active')->where('is_accept', '=', '0')->get();
    ...
    {
        $wprId = dec($id);
        $wprData = WorkProcessRequirement::where('id', $wprId)->where('status', 'Active')->first();
        $itemId = $wprData->item_id;
        $itemTypeId = $wprData->item_type_id;
        ...
        $dataWPR = WorkProcessRequirement::where('id', $wprId)->where('status', 'Active')->where('is_accept', 0)->with(['WorkOrderItem', 'Item'])->get();
        ...
        $processI = ProcessItem::where('status', '=', 'Active')->whereIn('id', [2, 3, 4])->get();
        $dataSO = StockMillDispatch::where('status', '=', 'Active')->count();
