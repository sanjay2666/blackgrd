...
class WorkOrderController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($workStatus == '2') {
            $query->where('inspection_status', InspectionStatus::Completed->value);
        }

        if (! empty($proceStatus)) {
            $query->whereNull('master_ind_id')->where('is_item_received_from_warehouse', '=', 'Yes')->where('is_work_require_request_accepted', '=', 'Yes')->where('inspection_status', InspectionStatus::Pending->value)->whereNotIn('execution_status', [WorkOrderExecutionStatus::Completed->value, WorkOrderExecutionStatus::Cancelled->value]);
    ...
    public function store(Request $request, TransitionSaleOrder $transitionSaleOrder)
    {
        ...
        try {
            ...
            foreach (array_values($saleOrderItemIds) as $index => $saleOrderItemId) {
                ...
                if ($saleOrder !== null && $saleOrder->document_status !== SaleOrderDocumentStatus::InProduction) {
                    $transitionSaleOrder->execute(
                        $saleOrder,
                        SaleOrderDocumentStatus::InProduction,
                        actorId: Auth::id(),
                    );
                }
            }
    ...
    public function start_requisition_process(string $id)
    {
        ...
        $resultArray = WarehouseItemStock::where('item_id', $itemId)->where('item_type_id', $itemTypeId)->where('entry_type', 'IN')->where('is_allotted_stock', 'No')->where('status', 'Active')->get();
    ...
    public function receive_work_item_in_warehouse(Request $request)
    {
        ...
        try {
            ...
            GatePass::whereIn('id', array_filter(array_map('intval', $gatePassNos)))->update([
                'is_item_received_in_warehouse' => 'Yes',
                'gate_pass_status' => GatePassStatus::Received->value,
            ]);
    ...
    public function closeWorkOrder(Request $request, TransitionWorkOrder $transitionWorkOrder)
    {
        // validate input
        $validator = Validator::make($request->all(), [
        ...
        try {
            ...
            $workData->inspection_status = InspectionStatus::Completed;
            $workData->insp_status = 'Complete';
            $workData->process_ended_by = $individualId;
            $isUpdate = $workData->save();
    ...
    public function updateinspectionworkorder(Request $request)
    {
        ...
        try {
            ...
            $curDate = now();
            if ($insp_status == 'Complete') {
                WorkOrder::where('id', $workOrderId)->update(['insp_status' => $insp_status, 'inspection_status' => InspectionStatus::Completed->value]);
                WorkOrder::where('id', $workOrderId)->update(['process_ended_date' => $curDate]);
                WorkOrder::where('id', $workOrderId)->update(['work_status' => $insp_status, 'execution_status' => WorkOrderExecutionStatus::Completed->value]);
            }
        ...
        try {
            ...
            if ($insp_status == 'Complete') {
                WorkOrder::where('id', $workOrderId)
                    ->update(['insp_status' => $insp_status, 'inspection_status' => InspectionStatus::Completed->value, 'process_ended_date' => $curDate, 'work_status' => $insp_status, 'execution_status' => WorkOrderExecutionStatus::Completed->value]);
            }
        ...
        try {
            ...
            if ($inspStatus == 'Complete') {
                WorkOrder::where('id', $workOrderId)->update([
                    ...
                    'inspection_status' => InspectionStatus::Completed->value,
                    'process_ended_date' => $curDate,
                    'work_status' => $inspStatus,
                    'execution_status' => WorkOrderExecutionStatus::Completed->value,
                ]);
            }
        ...
        try {
            ...
            if ($printPosition == 'after') {
                ...
                foreach ($woiSql as $row) {
                    ...
                    if (! empty($saleOrderItemId)) {
                        $chkNxtOrd = WorkOrderItem::where('sale_order_item_id', '=', $saleOrderItemId)->whereHas('WorkOrder', function ($query) use ($workOrderId) {
                            $query->where('parent_work_order_id', '=', $workOrderId);
                        })->with('WorkOrder')->where('status', '=', 'Active')->first();
                    }
                    ...
                    } elseif ($printJobVal == '' || ! empty($saleOrderItemId)) {
                        if (! empty($saleOrderItemId)) {
                            SaleOrderItem::where('id', $saleOrderItemId)->update(['is_work_completed' => 1]);
                        }
                    }
    ...
    public function workOrderTotals(Request $request, DepartmentAccessService $departmentAccess)
    {
        ...
        if ($workStatus === '' || $workStatus === '1') {
            ...
            $query->where('work_orders.inspection_status', InspectionStatus::Completed->value);
        }
        ...
        if ($proceStatus !== '') {
            $query->whereNull('work_orders.master_ind_id')
                ...
                ->where('work_orders.inspection_status', InspectionStatus::Pending->value)
                ->whereNotIn('work_orders.execution_status', [WorkOrderExecutionStatus::Completed->value, WorkOrderExecutionStatus::Cancelled->value]);
        }
    ...
    {
        $query = $request->query();
        $query['search_process_id'] = [3];

        return redirect()->route('show-workorders', $query);
    }
    ...
    public function show_workorder_inspection_report(Request $request, DepartmentAccessService $departmentAccess)
    {
        ...
        /* if (!empty($processTypeId)) {
            $workorderids = WorkOrder::where('process_type_id', '=', $processTypeId)->where('status', '=', 'Active')->pluck('work_order_id')->implode(',');
            $query->whereIn('work_order_id', explode(',', $workorderids));
        } */
