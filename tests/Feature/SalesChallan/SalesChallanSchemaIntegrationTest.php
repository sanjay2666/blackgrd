...
class SalesChallanSchemaIntegrationTest extends TestCase
{
    ...
    public function test_sales_challan_tables_preserve_header_item_and_physical_roll_traceability(): void
    {
        ...
        $this->assertTrue(Schema::hasColumns('sales_challan_items', ['sales_challan_id', 'packaging_order_id', 'packaging_order_item_id', 'sale_order_id', 'sale_order_item_id', 'item_name', 'grey_quality', 'dyeing_color', 'coating_type', 'final_dispatch_width', 'tube_width']));
        $this->assertTrue(Schema::hasColumns('sales_challan_roll_allocations', ['sales_challan_id', 'sales_challan_item_id', 'packaging_roll_allocation_id', 'warehouse_item_stock_id', 'dyeing_lot_number', 'packet_number', 'insp_taka_number', 'packed_quantity_snapshot', 'previously_dispatched_quantity_snapshot', 'dispatched_quantity']));
    }
    ...
    public function test_same_source_roll_cannot_be_registered_twice_on_one_challan(): void
    {
        ...
        $challanId = DB::table('sales_challans')->insertGetId([
            'company_id' => $suffix, 'customer_id' => $suffix, 'challan_number' => 'SC-'.$suffix, 'status' => 'Draft', 'challan_date' => now()->toDateString(), 'customer_name' => 'Schema Test', 'submission_key' => (string) \Illuminate\Support\Str::uuid(), 'record_status' => 'Active',
        ]);
